...
class GrainfatherOnTapCard extends HTMLElement {
  ...
  _collectRowsByStatuses(statuses) {
    ...
    for (const entityId of Object.keys(states)) {
      ...
      rows.push({ entityId, batchNumber, batchName, style, abv, notes });
    }
  ...
  _render() {
    ...
    const showOnTap = this._config.show_on_tap !== false;
    const showCommingSoon = this._config.show_coming_soon !== false;
    const showNotes = this._config.show_notes === true;

    const comingSoonStatuses = [];
    if (this._config.coming_soon_show_brewing !== false) {
      comingSoonStatuses.push('brewing');
    }
    if (this._config.coming_soon_show_fermenting !== false) {
      comingSoonStatuses.push('fermenting');
    }
    if (this._config.coming_soon_show_conditioning !== false) {
