...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    coordinator = GrainfatherDataUpdateCoordinator(hass, api)
...
def _resolve_batch_target(
    ...
    active_batch = coordinator.data.active_batch
    if active_batch is None:
        raise HomeAssistantError("No active Grainfather brew session is available")

    resolved_brew_session_id = brew_session_id or int(active_batch.batch_id)
    resolved_recipe_id = recipe_id

    if resolved_recipe_id is None and resolved_brew_session_id == active_batch.batch_id:
        resolved_recipe_id = active_batch.recipe_id

    if resolved_recipe_id is None:
        raise HomeAssistantError(
            "recipe_id is required when targeting a brew session other than the active one"
        )

    return resolved_recipe_id, resolved_brew_session_id
